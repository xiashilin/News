...
import com.kaku.colorfulnews.utils.SharedPUtils;
...
public class NewsDetailActivity extends BaseActivity implements NewsDetailView {
    ...
    public void setNewsDetail(NewsDetail newsDetail) {
        ...
        mTime = newsDetail.getPtime();
        if (!TextUtils.isEmpty(img = newsDetail.getImg().get(0).getSrc())) {
            img = newsDetail.getImg().get(0).getSrc();
        } else {
            img = "";
        }
...
//        mProgressBar.setVisibility(View.GONE);
        ...
        switch (item.getItemId()) {
            case R.id.action_browser:
                if (AVUser.getCurrentUser() != null) {
                    ...
                    ToastUtils.show(NewsDetailActivity.this, "请先登录");
                }
    ...
    private void Collect() {
        ...
        CollectBean collectBean = realm.createObject(CollectBean.class); // Create a new object
        collectBean.setLink(mShareLink);
        collectBean.setTime(mTime);
        collectBean.setImg(img);
        collectBean.setTitle(mNewsTitle);
